"""
Ridge regression neural prediction for Wendy's custom SAE checkpoint.
dict_size=5000, 4 groups (G0-G3, 1250 each), k=32, final norm patch tokens.
Conditions: Raw_B7, Raw_B9, Full, G0, G1, G2, G3 (spatially flattened across patches).
"""
...
OUTDIR   = Path('/n/holylfs06/LABS/kempner_fellow_binxuwang/Users/binxuwang/Projects/nhp_sae_analysis/results/ridge_wendy_ckpt')
OUTDIR.mkdir(parents=True, exist_ok=True)

CKPT_PATH  = BASE / 'ckpts_wendy_custom/ae_final.pt'
DICT_SIZE  = 5000
N_GROUPS   = 4
GROUP_SIZE = DICT_SIZE // N_GROUPS  # 1250
K          = 32
...
COND_NAMES = ['Raw_B7', 'Raw_B9', 'Full', 'G0', 'G1', 'G2', 'G3']
COND_LABELS = {
    'Raw_B7': 'Raw DINOv2 B7',
    'Raw_B9': 'Raw DINOv2 B9',
    'Full': 'Full SAE',
    'G0':   'G0 (long-range)',
    'G1':   'G1 (mid-range)',
    'G2':   'G2 (local)',
...
# ── Reuse helpers from run_ridge_pred ──────────────────────────────────────
def load_exclude_df():
...
def plot_psth(all_r, t_axis, nc_thresh, outpath):
    ...
    ax.set_title(f"Ridge Prediction — Wendy's SAE (dict=5000, 4 groups, flatten)\nNC≥{nc_thresh}", fontsize=12)
...
def plot_window_bars(all_r_early, all_r_late, nc_thresh, outpath):
    ...
    ax.set_title(f"Wendy's SAE — Early/Late Window Prediction (flatten)", fontsize=12)
...
def main():
    ...
    # Load / encode DINOv2 final norm tokens + raw B7/B9 (spatially flattened)
    FEAT_CACHE = CACHE_DIR / 'wendy_sae_features_flatten.pkl'
    if FEAT_CACHE.exists():
        print("Loading encoded features from cache...")
        with open(FEAT_CACHE, 'rb') as fh:
            features = pickle.load(fh)
    else:
        # --- Raw DINOv2 B7 / B9 (spatially flattened) ---
        print("Loading raw DINOv2 block7/9 patch tokens...")
        with open(CACHE_DIR / 'dinov2_spatial_block7.pkl', 'rb') as fh:
            b7 = pickle.load(fh)['blocks.7_patch_spatial'][:N_NSD]  # (N, 256, 768)
        with open(CACHE_DIR / 'dinov2_spatial_block9.pkl', 'rb') as fh:
            b9 = pickle.load(fh)['blocks.9_patch_spatial'][:N_NSD]  # (N, 256, 768)

        # --- SAE encoding of final-norm patch tokens ---
        DINO_CACHE = CACHE_DIR / 'dinov2_final_norm_patchtokens.pkl'
        print(f"Loading DINOv2 final-norm patch tokens from {DINO_CACHE}...")
        ...
        # Flatten spatial dimension for all conditions
        features = {
            'Raw_B7': b7.reshape(N_NSD, -1).astype(np.float32),   # (N, 256*768)
            'Raw_B9': b9.reshape(N_NSD, -1).astype(np.float32),   # (N, 256*768)
            'Full':   codes.reshape(N_NSD, -1),                    # (N, 256*DICT_SIZE)
        }
        for g in range(N_GROUPS):
            s, e = g*GROUP_SIZE, (g+1)*GROUP_SIZE
            features[f'G{g}'] = codes[:, :, s:e].reshape(N_NSD, -1)  # (N, 256*GROUP_SIZE)
    ...
    for si, mat_path in enumerate(mat_files):
        ...
        responses = neural['responses']   # (n_roi, n_time_total, N_NSD)
